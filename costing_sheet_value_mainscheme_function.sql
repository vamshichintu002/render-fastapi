CREATE OR REPLACE FUNCTION costing_sheet_value_mainscheme(p_scheme_id TEXT)
RETURNS TABLE (
  credit_account TEXT,
  customer_name TEXT,
  so_name TEXT,
  ...
  spent_per_value NUMERIC,
  estimated_value_final NUMERIC,
...
-- Base Period 1 Sales Data (using main scheme products)
...
-- Base Period 2 Sales Data (using main scheme products)
...
slab_applied AS (
  SELECT
    ...
    -- Target value based on base period data (VALUE-BASED CALCULATION)
    CASE 
      WHEN COALESCE(bs.total_value, 0) = 0 THEN fs.slab_start
      ELSE GREATEST((1 + COALESCE(sl_base.growth_rate, fs.growth_rate)) * COALESCE(bs.total_value, 0), COALESCE(sl_base.slab_start, fs.slab_start))
    END AS target_value,
    -- Estimated qualifiers
    (
      CASE 
        WHEN COALESCE(bs.total_value, 0) = 0 THEN fs.qualification_rate * 1
        ELSE COALESCE(sl_base.qualification_rate, fs.qualification_rate) * 1
      END
    ) AS estimated_qualifiers,
    -- Estimated value (VALUE-BASED CALCULATION)
    CASE 
      WHEN COALESCE(bs.total_value, 0) = 0 THEN fs.qualification_rate * fs.slab_start
      ELSE COALESCE(sl_base.qualification_rate, fs.qualification_rate) *
           GREATEST((1 + COALESCE(sl_base.growth_rate, fs.growth_rate)) * COALESCE(bs.total_value, 0), COALESCE(sl_base.slab_start, fs.slab_start))
    END AS estimated_value,
    -- Estimated volume (VALUE-BASED CALCULATION): estimated_volume = estimated_value / price_per_unit
    CASE 
      WHEN COALESCE(bs.total_volume, 0) = 0 THEN 0
      ELSE 
        (CASE 
          WHEN COALESCE(bs.total_value, 0) = 0 THEN fs.qualification_rate * fs.slab_start
          ELSE COALESCE(sl_base.qualification_rate, fs.qualification_rate) *
               GREATEST((1 + COALESCE(sl_base.growth_rate, fs.growth_rate)) * COALESCE(bs.total_value, 0), COALESCE(sl_base.slab_start, fs.slab_start))
        END) / (COALESCE(bs.total_value, 0) / NULLIF(COALESCE(bs.total_volume, 0), 0))
    END AS estimated_volume,
    -- Basic payout: Handle both per litre and percentage based rebates (VALUE-BASED CALCULATION)
    CASE 
    ...
    END AS basic_payout,
    -- Estimated basic payout: Handle both per litre and percentage based rebates (VALUE-BASED CALCULATION)
    CASE 
      WHEN COALESCE(sl_base.rebate_per_litre, fs.rebate_per_litre, 0) > 0 THEN
      ...
      ELSE
        COALESCE(sl_base.rebate_percent, fs.rebate_percent, 0) *
          (
  ...
  LEFT JOIN slabs sl_base ON COALESCE(bs.total_value, 0) BETWEEN sl_base.slab_start AND sl_base.slab_end
...
final_output AS (
  SELECT 
    ...
    estimated_value AS "Estimated Value",
    ...
    CASE WHEN estimated_value = 0 THEN 0 ELSE estimated_basic_payout / estimated_value END AS spent_per_value,
    CASE WHEN total_value = 0 THEN 0 ELSE estimated_value END AS estimated_value_final,
    CASE WHEN total_value = 0 THEN 0 ELSE (estimated_value / total_value) - 1 END AS percent_growth_planned,
    CASE WHEN estimated_value = 0 THEN 0 ELSE (estimated_basic_payout / estimated_value) END AS percent_spent,
...
unioned AS (
  ...
  SELECT
    ...
    COALESCE(SUM("Target Value"), 0),
    COALESCE(SUM("Estimated Value"), 0),
    ...
    CASE WHEN SUM("Estimated Value") = 0 THEN 0 ELSE SUM("Estimated Base Payout") / SUM("Estimated Value") END AS spent_per_value,
    COALESCE(SUM("Estimated Value"), 0) AS estimated_value_final,
    CASE WHEN SUM("Final Base Value") = 0 THEN 0 ELSE (SUM("Estimated Value") / SUM("Final Base Value")) - 1 END AS percent_growth_planned,
    CASE WHEN SUM("Estimated Value") = 0 THEN 0 ELSE SUM("Estimated Base Payout") / SUM("Estimated Value") END AS percent_spent,
    NULL,
    NULL,
    NULL,
    1 AS is_grand_total
