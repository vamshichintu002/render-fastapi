...
def execute_costing_function(scheme_id):
    ...
    try:
        ...
        # Execute the costing function
        logger.info(f"Executing costing_sheet_volume_mainscheme with params: ['{scheme_id}']")
        start_time = time.time()
        
        cur.execute("SELECT * FROM costing_sheet_volume_mainscheme(%s)", [scheme_id])
        rows = cur.fetchall()
        ...
        return {
            ...
            "data": {
                ...
                "function_name": "costing_sheet_volume_mainscheme",
                "params": [scheme_id],
                "scheme_id": scheme_id
            },
            "message": "Main scheme volume costing calculated successfully",
            "execution_time": execution_time,
        ...
        return {
            "success": False,
            "error": str(e),
            "message": f"costing_sheet_volume_mainscheme failed after 0.00s: {str(e)}"
        }

class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        """Handle POST requests for main scheme volume costing"""
        try:
            # Read request body
            ...
            result = execute_costing_function(scheme_id)
